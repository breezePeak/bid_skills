...
`automatic-numbering` 包括本次明确要求的标题编号纠正、独立题注补齐、题注位置调整及已定位正文图表引用的最小范围改写；表格结构/外观、图片媒体、分节仍冻结。不能把坏文档的编号形式当作基准。

清理图像像素里的外部题注、修复图内文字出框属于有证据的图内容修复，不得谎报为 `image-layout` 或“特殊例外”。实际修图后仍须语义、风格、最终页三方对照和逐图缺陷闭环。修改报告不构成图片修复。

流水线的文字和表格阶段执行已有范围保护；Agent 手动修改仍必须按每轮 scope 快照/比较。纯图位置或分页调整不得更换媒体。补题注和重绘须拆开执行，不能用大范围豁免放行所有变化。
