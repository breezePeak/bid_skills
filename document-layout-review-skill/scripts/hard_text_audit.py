...
through span heuristics, especially full-width ASCII quotation marks.
"""
...
def audit_paragraph(text: str, part: str, idx: int) -> list[Issue]:
    ...
    if any(ch in text for ch in FULLWIDTH_ASCII_QUOTES):
        out.append(Issue(
            "error", "fullwidth-ascii-quote", part, idx,
    ...
    if kind in {"chinese", "mixed"} and HAN_RE.search(text):
        if re.search(fr'["＂][^"＂\n]*[{HAN}][^"＂\n]*["＂]', text):
            out.append(Issue("error", "ascii-double-quote-in-chinese", part, idx,
                             "中文语境中引用中文内容时必须使用中文双引号“”。", excerpt(text)))
        if re.search(fr"['＇][^'＇\n]*[{HAN}][^'＇\n]*['＇]", text):
            out.append(Issue("error", "ascii-single-quote-in-chinese", part, idx,
                             "中文语境中引用中文内容时必须使用中文单引号‘’。", excerpt(text)))
        ...
        for m in re.finditer(r'["＂\'＇]', text):
            ...
            if HAN_RE.search(left + right):
                out.append(Issue("error", "ambiguous-ascii-quote-near-chinese", part, idx,
    ...
    if re.search(fr'(?<=[{HAN}])[ \u3000]+(?=[{HAN}])', text):
        out.append(Issue("error", "space-between-han", part, idx,
                         "普通中文正文中相邻中文字符之间存在无用空格。", excerpt(text)))
    if re.search(fr'[ \u3000]+(?=[{re.escape(CJK_CLOSE)}])', text):
        out.append(Issue("error", "space-before-cjk-punctuation", part, idx,
                         "中文标点前存在无用空格。", excerpt(text)))
    if re.search(fr'(?<=[{re.escape(CJK_OPEN)}])[ \u3000]+', text):
        out.append(Issue("error", "space-after-cjk-opening-punctuation", part, idx,
                         "中文起始标点后存在无用空格。", excerpt(text)))
    if re.search(r'(?:，，|。。|；；|：：|？？|！！|、、)', text):
        out.append(Issue("error", "duplicated-chinese-punctuation", part, idx,
                         "存在重复中文标点。", excerpt(text)))
    return out
...
def main() -> int:
    ...
    with zipfile.ZipFile(args.docx) as z:
        names = [n for n in z.namelist() if (
            ...
            (n.startswith('word/header') and n.endswith('.xml')) or
