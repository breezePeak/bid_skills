...
def replace_style_definitions(target_styles: bytes, template_styles: bytes, style_ids: set[str]) -> bytes:
    ...
    tdd = troot.find(Q("docDefaults")); sdd = sroot.find(Q("docDefaults"))
    if sdd is not None:
        if tdd is not None:
...
def role_for_paragraph(p: etree._Element, roles: dict[str, str], section_no: int, technical_bid: bool) -> str | None:
    # 表格优先。
    ...
    if technical_bid and section_no >= 3 and txt and not p.xpath(".//w:drawing|.//w:object|.//w:pict", namespaces=NS):
...
def main() -> int:
    ...
    with zipfile.ZipFile(a.template) as zt, zipfile.ZipFile(a.docx) as zd:
        tnames = set(zt.namelist()); dnames = set(zd.namelist())
        ...
        for p in root.xpath(".//w:p", namespaces=NS):
            role = role_for_paragraph(p, roles, sections.get(id(p), 1), a.technical_bid)
            if role is None:
                continue
            sid = roles.get(role)
            if not sid:
            ...
            if old_sid != sid:
                set_pstyle(p, sid); pstyle_changed += 1
            ...
            if role not in {"image_paragraph"}:
                ...
                override_removed += clear_run_font_size_overrides(p, clear_heading_emphasis=role.startswith("heading"))
            counts[role] = counts.get(role, 0) + 1
        target_files["word/document.xml"] = etree.tostring(root, xml_declaration=True, encoding="UTF-8", standalone=True)
